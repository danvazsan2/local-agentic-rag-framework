 ...
 *
 * Ingest files into the RAG system for a specific session.
 *
 * Request body:
 * {
 *   "session_id": "chat-uuid",
 *   "files": [
 *     { "name": "file.pdf", "content": "base64-encoded" }
 *   ],
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { session_id, files, llm_model } = body
    ...
    console.error("Error in custom-rag/ingest:", error)
